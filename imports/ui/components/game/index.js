...
import FreezePaddle from "../freezePaddle";
...
class Game extends Component {
  ...
  componentDidMount() {
    ...
    for (let i = 0; i < 50; i++) {
      this.paddles.push(
        new Paddle({
          ...
          freezePaddles: this.freezePaddles,
    ...
    this.freezePaddles.forEach(p => {
      p.generateXandY(this.freezePaddles);
    });
    for (let i = 0; i <= 8; i++) {
  ...
  renderfreezePaddles = () => {
    this.freezePaddles.forEach(p => {
      p.render(this.ctx, this.freezePaddles);
    });
  };
  renderRandomPaddles = () => {
    this.randomPaddles.forEach(p => {
